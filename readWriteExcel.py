...
def read_excel():
    # 打开文件
    wb = xlrd.open_workbook(r'工作簿1.xlsx')

    # 获取所有sheet的名字
    for i, sheet_name in enumerate(wb.sheet_names()):
        print('sheet_name_%s = %s' % (i+1, sheet_name))
    ...
    # 获取某一个位置的数据
    # 1 ctypes : 0 empty,1 string, 2 number, 3 date, 4 boolean, 5 error
    print(str('sheet1.cell(1, 0).ctype = ')+str(sheet1.cell(1, 0).ctype))
    print('sheet1.cell(1, 0).value = '+s)
    print('sheet1.cell(0, 0).value = ' + str(sheet1.cell(0, 0).value))
